...
export const StatisticsDashboard = () => {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [watched, favorites] = await Promise.all([getWatchedMovies(), getSavedMovies()]);

      // Calculate statistics
      const totalWatched = watched.length;
      const averageRating = totalWatched
        ? watched.reduce((acc, movie) => acc + (movie.rating || 0), 0) / totalWatched
        : 0;
