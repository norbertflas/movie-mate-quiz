...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.error('Gemini API error status:', geminiError.status);
      console.error('Gemini API error response:', geminiError.response);
      throw new Error(`Gemini API error: ${geminiError.status} - ${geminiError.response}`);
    }
  } catch (error) {
